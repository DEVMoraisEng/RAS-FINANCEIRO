# -*- coding: utf-8 -*-
"""
fetch_ras.py  (RAS FINANCEIRO)
------------------------------
Lê o banco de Atividades da RAS do Financeiro no Notion e gera o JSON que o
site consome:
...
Diferenças em relação ao fetch_ras.py da RAS geral:
  - não existe banco de Obras (a RAS do Financeiro não tem essa aba);
...
def fazer_curto(setores):
    ...
    ex.: 'Júlio César Gomes de Morais Filho') no nome curto que o site usa —
    que aqui é o próprio nome da opção de Setor ('Júlio César').
    ...
    Exemplos com os setores atuais:
        'Júlio César Gomes de Morais Filho' -> 'Júlio César'
        'Lohany Alves'                      -> 'Lohany'
        'Gustavo Dias Cabral'               -> 'Gustavo'
        'Ana Paula'                         -> 'Ana Paula'
        'Paula Morais'                      -> ''  (não é setor; mantém original)
